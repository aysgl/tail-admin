<template>
  ...
  >
    ...
    <template v-if="type === 'user'">
      <div
        class="size-8 shrink-0 animate-pulse overflow-hidden rounded-full bg-muted"
      />
      <div
    ...
    <template v-else-if="type === 'team'">
      <div class="flex -space-x-2">
        <div
          v-for="i in 3"
          :key="i"
          class="size-8 animate-pulse rounded-full border-2 border-white bg-muted dark:border-gray-800"
        />
      </div>
    </template>
    <!-- Status type: pill -->
    <template v-else-if="type === 'status'">
      <div
        class="h-6 w-16 animate-pulse rounded-full bg-muted"
      />
    </template>
    <!-- Default text type -->
    <template v-else>
      <div
        class="h-4 w-full max-w-[80%] animate-pulse rounded bg-muted"
      />
    </template>
  </div>
</template>
