...
1. [Formatlama Seçenekleri (Proje Standardı)](#1-formatlama-seçenekleri-proje-standardı)
2. [Dil / Dosya Türü Seçenekleri](#2-dil--dosya-türü-seçenekleri)
3. [Deneysel Seçenekler](#3-deneysel-seçenekler)
4. [Pragma / Kontrol Seçenekleri](#4-pragma--kontrol-seçenekleri)
5. [Eklentiler ve Override'lar](#5-eklentiler-ve-overridelar)
6. [API/CLI Özel Seçenekler](#6-apicli-özel-seçenekler)
...
- **Açıklama:** JSON Schema ile editörde otomatik tamamlama ve doğrulama (IDE desteği)
...
- **Varsayılan:** `false`
- **Değerler:** `true`, `false`
- **Açıklama:** `true` → HTML/JSX kapanış `>` son attribute ile aynı satırda; `false` → ayrı satırda
...
Farklı dosya türleri için farklı ayarlar tanımlanabilir (örn. JSON için farklı girinti). Projede
şu an özel override yok; tüm dosyalar (`.md` dahil) genel ayarları kullanır (`printWidth: 50`).
...
const { formatted, cursorOffset } =
  await prettier.formatWithCursor('  const x=1', {
    cursorOffset: 10,
    parser: 'babel',
  })
// formatted: 'const x = 1;\n'
